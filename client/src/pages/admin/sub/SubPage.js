...
function SubPage({ history, match }) {
  ...
  const loadSubs = () => getSubs().then((s) => setSubs(s));
    ...
    return (
      <Space size="small" align="start">
        <Typography.Title level={4}>
          {slug ? `Update ${sub.name}` : "Create new subcategory"}
        </Typography.Title>
        {slug ? (
          <Link to="/admin/sub">
  ...
  const renderForm = () => {
    return (
      <Form
        form={form}
        onFinish={slug ? handleEdit : handleCreate}
        layout="inline"
        requiredMark={false}
        size="large"
      >
        ...
        <Col flex="auto">
          ...
          <Card>
            ...
            <SubTable
              ...
              data={subs.filter(searched(keyword))}
